  ...
  <div class="flex flex-col gap-4">
    <div class="bg-opacity-5 bg-white shadow sm:rounded-lg mt-4">
      <div class="px-4 py-5 sm:p-6">
        <h3 class="text-lg leading-6 font-medium text-gray-100">{{ translations.player.title }}</h3>
        <div class="mt-2 max-w-xl text-sm text-gray-200">
          <p>{{ translations.player.desc }}</p>
        </div>
      </div>
    </div>
    <div class="px-3 flex flex-col gap-4 mt-4">
      <settings-checkbox config-key="player.skipDisliked"
        ...
        <div class="flex flex-col">
          <div>{{ translations.player.skipDisliked }}</div>
          <div class="select-none text-red-500 opacity-80 group-hover:opacity-100 uppercase text-xs font-medium">{{ translations.generic.experimental }}</div>
        </div>
      </settings-checkbox>
      <settings-checkbox config-key="volumeRatio.enabled"
                         class="group">
        <div class="flex flex-col">
          <div>{{ translations.player.volumeRatio }}</div>
          <div class="mt-2 max-w-xl text-sm text-gray-200">
            <p>{{ translations.player.volumeRatioDesc }}</p>
          </div>
        </div>
      </settings-checkbox>
      <div class="flex flex-col gap-4 -mx-3 px-3 py-3 rounded-lg border"
        ...
        <settings-checkbox config-key="player.res.enabled"
                           class="group">
          <div class="flex flex-col">
            <div>{{ translations.player.videoSettings }}</div>
            <div class="select-none opacity-80 group-hover:opacity-100 uppercase text-xs font-medium">{{ translations.player.videoSettingsDesc }}</div>
          </div>
        </settings-checkbox>
        <template v-if="resEnabled">
          <settings-select config-key="player.res.prefer"
                           class="bg-transparent border-0">
            <template #label>{{ translations.player.preferredResolution }}</template>
            <template #options>
      ...
      <settings-checkbox config-key="app.enableTaskbarProgress">
        <div class="flex flex-col">
          <span>{{ translations.player.taskbarProgress }}</span>
          <span class="opacity-80 text-xs">{{ translations.player.taskbarProgressDesc }}</span>
        </div>
      </settings-checkbox>
    </div>
  </div>
